...
\myprocedure[linenumbering]{$\Gamma_1(\mathsf{fSub}).\mathsf{Alg}_\lambda(x, ek, \tau, \mu)$}{
	...
	\pcelse \\
	\pcind ((\sigma, \mu), \tau') \leftarrow \tau \\
...
Let $b_\text{PKE}$ denote the bit from the IND-CPA$_\mathsf{PKE}(\mathcal{D}_2)$ game. Note that if $b_\text{PKE}=1$, then the oracle simulated by $\mathcal{D}_2$ proceeds exactly as in game $K_1$, since in $K_1$ the only value of $\mu$ passed to the $\mathsf{Helper}$ function is 0. If $b_\text{PKE}=0$, then $\mathcal{D}_2$ proceeds exactly as in game $K_0$. Thus we have
\shortlongeqn[,]{
